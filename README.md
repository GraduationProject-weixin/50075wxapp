...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1BPtKekEv3?p=78)


# 第1章 绪论
## 1.1 课题背景
随着移动互联形式的不断发展，各行各业都在摸索移动互联对本行业的改变，不断的尝试开发出适合于本行业或者本公司的APP。但是这样一来用户的手机上就需要安装各种软件，但是APP作为一个只为某个公司服务的一个软件，是一种闭环的生态，从用户获取和保持用户的粘性都变成了一个难题，并且在维护上面也需要考虑成本，慢慢的变成了不开发APP跟不上潮流，开发APP则运营上面步履维艰。如何获取一个新的客户来源通道以及转换通道，变成了管理者都需要面对的一个问题。如何让一个用户从已经习惯的应用里跳转到希望用户访问的应用里，这是一个问题，而微信小程序就解决了这样的难题。微信拥有庞大的用户基础，可以不用更换APP直接在微信上就可以访问用户想要访问的资源。因此微信小程序相对于APP来讲更受欢迎，本次课题就运用了微信小程序技术开发一个基于微信小程序的家政服务管理系统。
## 1.2 课题意义 
背靠微信庞大用户群体的微信小程序，变成了当前解决用户访问应用程序入口方案的一种选择，用户的手机里面只要有微信，不需要安装新的软件，就可以直接访问指定目标，获取指定内容，解决了管理者的痛点，并且微信作为一个聊天工具，在微信里有支持的各种各样的生活服务，也为微信的用户提供了粘性，微信小程序不管是针对用户或者管理者都是一个双赢的存在。目前，传统电商及零售行业因为小程序的出现而获得了新生。本次课题针对家政服务方面的信息，借助于微信小程序这样的平台，设计一个基于微信小程序的家政服务管理系统。使得用户可以更加方便预约家政服务，并在线支付预约的家政服务，家政阿姨也能审核家政预约信息，查看家政预约的相关评价信息。
## 1.3 研究内容
对基于微信小程序的家政服务管理系统设计制作，不仅需要技术支撑，也需要大量的理论研究。本文在对基于微信小程序的家政服务管理系统进行介绍时，将按照如下内容进行。

第一部分：介绍基于微信小程序的家政服务管理系统研究的背景意义，便于用户了解系统；

第二部分：介绍开发基于微信小程序的家政服务管理系统需要搭建的环境，包括技术和工具；

第三部分：介绍用户对基于微信小程序的家政服务管理系统的功能要求，以及对基于微信小程序的家政服务管理系统的性能要求等；
...
# 第2章 开发环境与技术
基于微信小程序的家政服务管理系统的编码实现需要搭建一定的环境和使用相应的技术，接下来的内容就是对基于微信小程序的家政服务管理系统用到的技术和工具进行介绍。
## 2.1 MYSQL数据库
...
### 3.1.2 经济可行性
本系统开发期间需要配置的软件环境，可以免费通过开发类官网下载安装，需要配置的硬件设备也不需要具备很高的性能，通常网吧电脑，或学校计算机机房的电脑都符合要求。因此，从经济方面考虑，基于微信小程序的家政服务管理系统开发可行。
### 3.1.3 操作可行性
基于微信小程序的家政服务管理系统根据用户使用习惯进行开发，设计的界面具有统一性，并具备优秀的导航功能。所以，操作人员可以无压力操作基于微信小程序的家政服务管理系统。
...
基于微信小程序的家政服务管理系统经过分析，确定了其需要设置管理员的角色，其操作的功能通过用例图展示（见下图）。管理员管理家政公司，家政阿姨，家政预约以及服务项目，管理评价和留言。

![](/md/blog.005.png)

图3.5 管理员用例图

基于微信小程序的家政服务管理系统经过分析，确定了其需要设置家政阿姨的角色，其操作的功能通过用例图展示（见下图）。家政阿姨审核家政预约信息，查看家政公司和评价。

![](/md/blog.006.png)

图3.6 家政阿姨用例图

基于微信小程序的家政服务管理系统经过分析，确定了其需要设置用户的角色，其操作的功能通过用例图展示（见下图）。用户预约家政服务并支付，评价预约的家政服务。
...
## 4.2 功能结构设计
在前面分析的管理员功能的基础上，进行接下来的设计工作，最终展示设计的管理员结构图（见下图）。管理员管理家政公司，家政阿姨，家政预约以及服务项目，管理评价和留言。

![](/md/blog.008.png)

图4.1 管理员功能结构图

在前面分析的家政阿姨功能的基础上，进行接下来的设计工作，最终展示设计的家政阿姨结构图（见下图）。家政阿姨审核家政预约信息，查看家政公司和评价。

![](/md/blog.009.png)

图4.2 家政阿姨功能结构图

在前面分析的用户功能的基础上，进行接下来的设计工作，最终展示设计的用户结构图（见下图）。用户预约家政服务并支付，评价预约的家政服务。
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|jiazhengzhanghao|varchar(200)|否||家政账号|
|mima|varchar(200)|否||密码|
|jiazhengxingming|varchar(200)|否||家政姓名|
|xingbie|varchar(200)|是|NULL|性别|
|nianling|varchar(200)|是|NULL|年龄|
|shouji|varchar(200)|是|NULL|手机|
|zhaopian|varchar(200)|是|NULL|照片|
|gongzuojingyan|longtext|是|NULL|工作经验|
表4.3 家政公司信息表
...
|jiazhengzhanghao|varchar(200)|是|NULL|家政账号|
|jiazhengxingming|varchar(200)|是|NULL|家政姓名|
|fuwuneirong|varchar(200)|是|NULL|服务内容|
...
|jiazhengzhanghao|varchar(200)|是|NULL|家政账号|
|jiazhengxingming|varchar(200)|是|NULL|家政姓名|
|fuwujiage|varchar(200)|是|NULL|服务价格|
|yuyueshijian|date|是|NULL|预约时间|
|shangmendizhi|varchar(200)|是|NULL|上门地址|
